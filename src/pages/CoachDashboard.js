...
const TabRequests = ({ applications, onApprove, onDecline }) => {
  ...
  const handleApprove = async (application) => {
    ...
    setLinkById(prev => ({ ...prev, [application.id]: { link: result.link, bookingLink: result.bookingLink } }))
  }
      ...
      {visible.map(application => (
        <div key={application.id} style={{ ...S.card, display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {linkById[application.id] ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {linkById[application.id].bookingLink && (
                <div style={{ background: badge('success').background, borderRadius: 8, padding: 12 }}>
                  <div style={{ fontSize: type.label, color: color.textOnLight.secondary, marginBottom: 6 }}>
                    Approved — send this link so they can pick an intake call time:
                  </div>
              ...
              <div style={{ background: color.surfaceSunken, borderRadius: 8, padding: 12 }}>
                <div style={{ fontSize: type.label, color: color.textOnLight.secondary, marginBottom: 6 }}>
                  Client account invite link (once they're ready to sign up):
                </div>
                <code style={{ fontSize: type.label, wordBreak: 'break-all', color: color.textOnLight.primary }}>
                  {linkById[application.id].link}
                </code>
              </div>
            </div>
          ) : (
...
export default function CoachDashboard() {
  ...
  const handleApproveApplication = async (application) => {
    ...
    // booking_token is generated by a DB default on every application row
    // (see the intake-scheduling migration), not something set here — the
    // link is only meaningful once approved, so it's only surfaced now.
    const bookingLink = data.booking_token ? `${window.location.origin}/book/${data.booking_token}` : null
    return { ok: true, link: invite.link, bookingLink }
  }
