...
export default function PublicApply({ slug }) {
  ...
  if (status === 'loading') return <Shell><p style={{ color: color.textOnDark.secondary, fontSize: type.body, margin: 0 }}>Loading...</p></Shell>;
  if (status === 'invalid') return <Shell><p style={{ color: color.textOnDark.secondary, fontSize: type.body, margin: 0 }}>This application link isn't valid.</p></Shell>;

  if (status === 'submitted') {
    return (
      <Shell>
        <h2 style={{ fontWeight: 500, fontSize: type.heading, color: color.textOnDark.primary, margin: '0 0 8px' }}>Application received</h2>
        <p style={{ color: color.textOnDark.secondary, fontSize: type.body, margin: 0 }}>
  ...
  return (
    <Shell>
      <h2 style={{ fontWeight: 500, fontSize: type.heading, color: color.textOnDark.primary, margin: '0 0 8px' }}>Apply to work together</h2>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div>
          <label style={labelStyle(true)}>Phone (optional)</label>
          <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} style={inputStyle} />
        </div>
        <div>
          <label style={labelStyle(true)}>Goals / notes</label>
