...
- Coach-branded application link with service selection and fit questions — extends `PublicApply.jsx`, doesn't replace it. **✅ SHIPPED (2026-09-13):** converted `PublicApply.jsx` from a single flat form to a 6-step questionnaire (Basics/Goals/Training/Nutrition/Logistics/Notes), reusing `CheckInForm.js`'s step-UI pattern — extracted `SectionHeader`/`StepProgress`/`StepNav` into a shared `StepFlow.jsx` rather than duplicating it, so the two forms can't drift apart. Adds 4 grouped `jsonb` columns to `client_applications` (`goals`, `training`, `nutrition`, `logistics`), each with a defensive `pg_column_size() < 10000` CHECK constraint (the INSERT policy is public/unauthenticated, so nothing else bounded payload size before this). Coach-side Requests tab now displays the new fields grouped by section, matching the form's structure. Existing RLS confirmed byte-for-byte unchanged by both migrations. No photo/file upload — explicitly out of scope for this pass.
...
**Adjacent, pre-existing infrastructure closed a real gap this session (not itself a Phase 17 bullet, but investigated and extended while working this area):** the coach's intake-call booking flow (`BookIntakeCall.jsx`, `coach_availability_rules`, `intake_call_bookings`) predates this phase plan entirely (commits `e3728cc`/`a6fe802`/`41ef928`/`5db8d6f`, July 2026) — a real, custom-built slot scheduler (coach-defined weekly availability → generated 20-minute slots → conflict-checked booking → confirmation emails to both sides), not Calendly or a text field. Investigation confirmed exactly one gap: coaches had zero in-app visibility into their own bookings, only a one-time confirmation email. **✅ SHIPPED (2026-09-13):** read-only "Upcoming intake calls" section added to the Requests tab (name + formatted date/time, today-and-future only, no booking/cancel/edit actions) — existing RLS (`coach_id = auth.uid()` on `intake_call_bookings`) already covered the read, re-verified rather than assumed, no new policy needed. **Confirmed still a gap, not addressed this pass:** sending the booking link to an approved applicant is entirely manual — the coach copy-pastes it themselves after Approve, no automated send.
